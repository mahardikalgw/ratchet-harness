...
async fn append_and_load_round_trips() {
    ...
    store.append(&metric("T-1", "deepseek", 0.01, true)).await.unwrap();
    store.append(&metric("T-2", "claude", 0.50, false)).await.unwrap();
...
async fn aggregation_sums_tokens_and_cost() {
    ...
    store.append(&metric("T-1", "deepseek", 0.01, true)).await.unwrap();
    store.append(&metric("T-2", "deepseek", 0.02, true)).await.unwrap();
    store.append(&metric("T-3", "claude", 0.50, false)).await.unwrap();

    let agg = store.aggregate_since(Utc::now() - Duration::hours(1)).await.unwrap();
